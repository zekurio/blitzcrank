...
import { buildServiceTools, isReadTool, type SessionFileRef } from "../tools/index.js";
...
export function parseAutomationOutput(text: string): Omit<AutomationReport, "name"> {
  const trimmed = text.trim();
  if (trimmed === "") return { status: "ok", body: "", empty: true, malformed: false };

  const lines = trimmed.split("\n");
  const status = lines[0]!.trim().match(/^STATUS:\s*(ok|warnung|fehler)\s*$/i);
  if (!status) return { status: "fehler", body: trimmed, empty: false, malformed: true };
...
export class AutomationRunner {
  ...
  async run(def: AutomationDefinition): Promise<AutomationReport> {
    ...
    const report: AutomationReport = { name: def.name, ...parseAutomationOutput(finalText) };
