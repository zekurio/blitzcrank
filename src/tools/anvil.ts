...
function execFileText(
  ...
  return new Promise((resolve, reject) => {
    execFile(file, args, { ...(signal ? { signal } : {}), timeout: 10_000, maxBuffer: 1024 * 1024 }, (error, stdout, stderr) => {
      if (error) {
  ...
  return [
    defineTool({
      ...
      parameters: Type.Object({
        purpose: Type.String({ description: "Why Anvil daemon health is needed for this diagnosis" }),
      }),
      async execute(_toolCallId, _params, signal) {
        const stdout = await execFileText(cfg.command, ["--socket", cfg.socket, "status", "--json"], signal);
        return textResult(parseAnvilResponse(stdout), { action: "anvil_status" });
      },
    }),
    defineTool({
      ...
      async execute(_toolCallId, params, signal) {
        ...
        const stdout = await execFileText(
          cfg.command,
          ["--socket", cfg.socket, "job", "list", "--absolute-path", path, "--current-only", "--json"],
          signal,
        );
        return textResult(parseAnvilResponse(stdout), { action: "anvil_job_lookup" });
      },
    }),
  ];
}
