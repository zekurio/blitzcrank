...
export function buildSabnzbdTools(
  ...
  return [
    ...
    defineTool({
      ...
      description:
        "Remove one job from the SABnzbd queue or history. deleteFiles=true also deletes downloaded data (the issue-wide deletion budget applies). Prefer Arr-level queue removal when the Arr still tracks the item; never orphan an Arr that is waiting on this job. The nzo_id must come from a SABnzbd read on this issue.",
      parameters: Type.Object({
        reason: reasonParam(),
