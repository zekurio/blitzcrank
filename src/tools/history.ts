...
export function buildHistoryTool(
  ...
  return defineTool({
    ...
    parameters: Type.Object({
      query: Type.String({
        description: "Search terms such as a title, error, queue/import symptom, or prior fix",
      }),
      source: Type.Optional(StringEnum(["all", "issues", "automations"] as const)),
    ...
    async execute(_toolCallId, params) {
      ...
      const roots =
        source === "all"
          ? [path.join(sessionsRoot, "issues"), path.join(sessionsRoot, "automations")]
      ...
      for (const file of files) {
        if (currentSessionFile.current && file === currentSessionFile.current) continue;
        let text: string;
        try {
