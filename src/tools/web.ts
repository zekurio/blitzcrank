    ...
    defineTool({
      ...
      async execute(_toolCallId, params) {
        const result = await firecrawl<SearchResponse>(
          "/v2/search",
          { query: params.query, limit: params.limit ?? 5, sources: ["web"], timeout: 30_000 },
          35_000,
        );
        ...
        return textResult({ query: params.query, results: hits }, { action: "web_search" });
      },
    }),
    defineTool({
      name: "web_fetch",
      ...
      async execute(_toolCallId, params) {
        ...
        const result = await firecrawl<ScrapeResponse>(
          "/v2/scrape",
          { url, formats: ["markdown"], onlyMainContent: true, timeout: 30_000 },
          40_000,
        );
