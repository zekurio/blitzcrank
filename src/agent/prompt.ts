...
const CAPABILITY_LINES: ReadonlyArray<readonly [string, string]> = [
  ...
  one complete exact Arr queue \`outputPath\` match and read \`anvil_job_show\`;
  retrying does not fix a cause.`,
  ],
  [
    "anvil_job_show",
...
export function buildSystemPrompt(
  ...
  const anvilRules = toolNames.includes("anvil_status")
    ? `
- Daemon health does not prove a download is encoding. Correlate pre-import work only by
  ...
  matching Arr \`downloadId\` to \`nzo_id\`. Diagnostics may also use an exact imported-file
  path, or a converted path returned by Anvil in this run. Never carry paths across runs;
  if none is currently service-supplied, do not guess.
...
- If verifiable work remains pending, emit \`REVISIT_IN\` (the host clamps it to 10m–48h)
  ...
  use 10–15m for nearly complete downloads or running encodes, and hours for barely
  started or ungrabbed work. Do not schedule while waiting for the reporter; their comment
  wakes you.
