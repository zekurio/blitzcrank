...
- Remove a verified bad queue item: call `sonarr_delete_queue_item` with the verified `queueId` and explicit `blocklist` and `removeFromClient` booleans. With `removeFromClient: true` the downloaded data is destroyed and the call is recorded as a deletion; with `removeFromClient: false` nothing is destroyed.
...
- Delete one verified wrong episode file only after replacement is confirmed by the reporter: call `sonarr_delete_episode_file` with the verified `episodeFileId`. Preserve multi-episode relationships, then search only the affected episode. When a whole season is verified wrong, delete every affected file rather than a sample: nothing caps the count, and leaving part of a bad season in place is a worse library state than either finishing or not starting. Establish the full extent first and name it to the reporter.
