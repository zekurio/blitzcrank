...
- If local evidence shows that the requested audio/subtitle track is absent from imported files, continue local research before using `web_search`: inspect the relevant Sonarr/Radarr history, queue, blocklist, imported file metadata, quality profile/language/custom-format evidence, and narrow release/search results when needed to determine whether a better local candidate exists or whether prior candidates were rejected. Use `web_search` only after those local checks cannot explain the blocker and the final answer truly depends on external release/provider facts.
