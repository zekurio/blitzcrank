...
Anvil transcodes between SABnzbd completion and Arr import. When registered,
...
Blitzcrank can read Anvil when those tools are present and can requeue one job
only when `anvil_retry_job` is present. It cannot cancel, abort, pause, resume,
...
slug to its immutable numeric ID. Before retrying, the same job must be the sole
match from a complete lookup of an exact current Arr queue `outputPath` and
must be inspected with `anvil_job_show` in this run. It rejects a second retry in one run and every
canceled, active, complete, skipped, or `retrying` job.

Use `anvil_job_show` and ensure the cause is gone. Missing
...
- `anvil_job_list`, narrowed by `states`, for snapshots. Absence is established
  only with `truncated: false`, `matched` equal to the returned job count, no
  `output_complete: false`, and no Blitzcrank truncation marker.
...
- `anvil_job_show` for any failed, expired-lease, or implausibly long job, or an
  already evidenced historical numeric ID/current-run slug.
...
truncated output. Retry requires one unique exact current Arr queue `outputPath`
match; a package or otherwise multiple match stays read-only and needs operator
review. Exact active-job evidence **plus** Arr file-not-ready evidence
