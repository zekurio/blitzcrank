...
Use `anvil_status` for daemon health and `anvil_job_lookup` for exact item correlation. Both tools are unchanged, read-only, and require `purpose`; `anvil_job_lookup` also requires the exact `absolute_path`. Daemon health never proves that a particular media item is encoding.
