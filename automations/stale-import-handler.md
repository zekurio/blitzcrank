...
3. Take one `anvil_job_list` snapshot for `pending`, `leased`, `running`, `validating`, `replacing`, and `retrying`; reuse it for all candidates. Absence is meaningful only when `truncated: false`, `matched` equals the returned job count, there is no `output_complete: false`, and there is no blitzcrank truncation marker.
...
For a healthy Anvil wait, perform no Arr or SABnzbd mutation. Do not list it under manual review. If all candidates are healthy waits and nothing changed, finish with `status: "ok"` and an empty report body.

### Retry one failed Anvil job

Call `anvil_retry_job` only when all are true:

- one complete lookup of the exact current Arr queue `outputPath` uniquely matched the job;
- the id/slug appeared verbatim in an Anvil read this run;
- `anvil_job_show` was called this run, is complete and reports `failed` with a plausibly transient attempt error such as recovered I/O or a worker dying mid-encode;
