...
Snapshot: [`zekurio/anvil` at `02bad6e`][snapshot], verified against upstream
`main` on 2026-08-24. Anvil has no tags, releases, or changelog, so the commit
is the compatibility reference; package version `0.1.0` and API version `v1`
do not identify this interface precisely.

[snapshot]: https://github.com/zekurio/anvil/tree/02bad6e49b9fbb28d6baa537459586ac7a6c9d56
...
[operating]: https://github.com/zekurio/anvil/blob/02bad6e49b9fbb28d6baa537459586ac7a6c9d56/README.md#operating
[cli]: https://github.com/zekurio/anvil/blob/02bad6e49b9fbb28d6baa537459586ac7a6c9d56/cmd/anvilctl/main.go
[types]: https://github.com/zekurio/anvil/blob/02bad6e49b9fbb28d6baa537459586ac7a6c9d56/pkg/control/types.go
[show-types]: https://github.com/zekurio/anvil/blob/02bad6e49b9fbb28d6baa537459586ac7a6c9d56/pkg/control/types_job_detail.go
[lifecycle]: https://github.com/zekurio/anvil/blob/02bad6e49b9fbb28d6baa537459586ac7a6c9d56/pkg/store/sqlite_job_lifecycle.go
[slugs]: https://github.com/zekurio/anvil/blob/02bad6e49b9fbb28d6baa537459586ac7a6c9d56/pkg/store/job_slugs.go
[resume]: https://github.com/zekurio/anvil/blob/02bad6e49b9fbb28d6baa537459586ac7a6c9d56/pkg/worker/resume.go
[worker]: https://github.com/zekurio/anvil/blob/02bad6e49b9fbb28d6baa537459586ac7a6c9d56/pkg/worker/worker.go
[scanner]: https://github.com/zekurio/anvil/blob/02bad6e49b9fbb28d6baa537459586ac7a6c9d56/pkg/scanner/filesystem_linux.go
...
A listing is complete only when `truncated` is explicitly false, `matched`
equals the returned job count, and the rendered response fits. Anything else
can never prove even snapshot absence. A complete result can say only that no
matching job is present at that moment, not that one cannot be
...
Diagnostics have their own rendered-size budget. If any failure diagnostic is
omitted, or if even the compact core does not fit pi's result cap, the tool
returns `output_complete: false` and an explicit no-retry conclusion. The
oversized-core fallback keeps recent bounded attempt summaries; the mutation
tool enforces both conclusions.
...
current-run slug paired with that ID. This run must also have uniquely matched
that job through one complete lookup of an exact current Sonarr/Radarr queue
`outputPath` and exposed its diagnostics through `anvil_job_show`; a broad
listing is insufficient. The mutation is
evidence-gated through `runMutation`, limited to one attempt per job per run,
recorded for audit, and verified with `show`.
...
before spending the mutation. It marks the retry attempted before contacting
the daemon, because a transport failure may hide a successful state change and
must not cause a duplicate retry in the same run. Canceled jobs are
operator-only: a canceled publish conflict may already have written a destination or left backup/cleanup
