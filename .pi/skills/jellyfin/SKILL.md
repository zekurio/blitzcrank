...
- Item by id with media sources and external identity: `GET /Items/{itemId}?Fields=MediaSources,Path,ProviderIds`
...
- Use `ProviderIds` such as TVDB, TMDB, IMDb, or AniDB to disambiguate web research and downstream service lookups. Treat missing provider ids as missing evidence, not permission to guess.
