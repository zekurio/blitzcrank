...
- Manual import a verified candidate: `POST /api/v3/command` with body shaped like Sonarr `ManualImport`; use `importMode: "Move"` and `force: true` only when the only blocker is a stale queue/import warning and the candidate is otherwise clearly correct.
